...
function findLatestPluginLog() {
    if (!fs.existsSync(VSCODE_LOG_BASE))
        return undefined;
    const sessions = fs
    ...
    for (const session of sessions) {
        ...
        for (const win of windows) {
            const logPath = path.join(sessionDir, win, 'exthost', 'Anthropic.claude-code', 'Claude VSCode.log');
...
function parsePluginLog(logPath) {
    ...
    for (const line of lines) {
        ...
        const sessionMatch = line.match(/"type":"update_session_state","sessionId":"([^"]+)","state":"([^"]+)","title":"([^"]*)"/);
        if (sessionMatch) {
        ...
        if (effortMatch)
            state.effortLevel = effortMatch[1];
        // thinking
        const thinkingMatch = line.match(/"thinking":\s*(true|false)/);
        if (thinkingMatch)
            state.thinking = thinkingMatch[1] === 'true';
        ...
        if (setModelMatch) {
            ...
            else if (v === 'opus' || v === 'sonnet' || v === 'haiku')
                state.model = `claude-${v}`;
            else
                state.model = v;
        }
...
function activate(context) {
    ...
    function update() {
        ...
        statusBarItem.tooltip = new vscode.MarkdownString(`**Claude VS Code Plugin**\n\n` +
    ...
    const selectCmd = vscode.commands.registerCommand('claudeStatusLine.selectSession', async () => {
        if (!logPath)
            return;
        const state = parsePluginLog(logPath);
        const all = [...state.sessions.values()].sort((a, b) => b.lastUpdate - a.lastUpdate);
        ...
        const choice = await vscode.window.showQuickPick(all.map((s) => ({
